...
function drawList() {
  ...
  root.innerHTML = `<div class="page-head"><div><h2>${jobs.length} videos</h2><p>Analysed by ${esc(ctx.overview.system.model)} · ${ctx.overview.jobs.minutes_of_video} minutes of footage</p></div><span class="sp"></span>
    <div class="row"><input type="search" id="vq" placeholder="filter by name or id" value="${attr(q)}" style="width:min(240px,100%)"><button class="btn filled" id="analyse">${icon('add')}Analyse a video</button></div></div>
