...
function pickFolderDialog(nas) {
  ...
  async function load(path) {
    ...
    const detail = (f) => f.videos ? f.videos + ' video' + (f.videos === 1 ? '' : 's') + ' inside' : data.places && !f.folders && f.path !== '/' ? (data.docker ? 'empty — nothing mounted here on the server yet' : 'empty') : data.places && f.folders ? f.folders + ' folder' + (f.folders === 1 ? '' : 's') : 'no videos directly inside';
    list.innerHTML = data.folders.map((f) => `<div class="li clickable" data-p="${attr(f.path)}"><div class="avatar">${icon(f.mounted ? 'dns' : 'folder')}</div><div class="grow"><div class="t">${esc(f.name)}</div><div class="d">${detail(f)}${f.mounted ? ' · mounted share' : ''}${already.has(f.path) ? ' · already watched' : ''}</div></div>${icon('chevron')}</div>`).join('')
      ...
      || `<div class="empty">${data.places ? 'No mounted locations found. Mount the NAS on this machine (see docs/DEPLOY.md) or set NAS_WATCH_DIR.' : data.docker ? 'empty — inside Docker only mounted host folders are visible; mount the NAS under /mnt or /media on the server' : 'empty folder'}</div>`;
